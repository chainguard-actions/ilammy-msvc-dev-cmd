...
var require_lib2 = __commonJS({
  "lib.js"(exports2) {
    ...
    var PROGRAM_FILES_X86 = process2.env["ProgramFiles(x86)"];
    var PROGRAM_FILES = [process2.env["ProgramFiles(x86)"], process2.env["ProgramFiles"]];
    var EDITIONS = ["Enterprise", "Professional", "Community", "BuildTools"];
    var YEARS = ["2022", "2019", "2017"];
    var VsYearVersion = {
      "2022": "17.0",
      "2019": "16.0",
      "2017": "15.0",
    ...
    exports2.vsversion_to_versionnumber = vsversion_to_versionnumber;
    ...
    exports2.vsversion_to_year = vsversion_to_year;
    var VSWHERE_PATH = `${PROGRAM_FILES_X86}\\Microsoft Visual Studio\\Installer`;
    function findWithVswhere(pattern, version_pattern) {
      try {
        ...
        core2.warning(`vswhere failed: ${e}`);
      }
      return null;
    }
    exports2.findWithVswhere = findWithVswhere;
    function findVcvarsall(vsversion) {
      ...
      let path2 = findWithVswhere("VC\\Auxiliary\\Build\\vcvarsall.bat", version_pattern);
      if (path2 && fs.existsSync(path2)) {
        core2.info(`Found with vswhere: ${path2}`);
        return path2;
      }
      core2.info("Not found with vswhere");
      const years = vsversion ? [vsversion_to_year(vsversion)] : YEARS;
      for (const prog_files of PROGRAM_FILES) {
        for (const ver of years) {
          for (const ed of EDITIONS) {
            path2 = `${prog_files}\\Microsoft Visual Studio\\${ver}\\${ed}\\VC\\Auxiliary\\Build\\vcvarsall.bat`;
            core2.info(`Trying standard location: ${path2}`);
            if (fs.existsSync(path2)) {
              core2.info(`Found standard location: ${path2}`);
              return path2;
            }
          }
        }
      }
      core2.info("Not found in standard locations");
      path2 = `${PROGRAM_FILES_X86}\\Microsoft Visual C++ Build Tools\\vcbuildtools.bat`;
      if (fs.existsSync(path2)) {
        core2.info(`Found VS 2015: ${path2}`);
        return path2;
      }
      core2.info(`Not found in VS 2015 location: ${path2}`);
      throw new Error("Microsoft Visual Studio not found");
    }
    exports2.findVcvarsall = findVcvarsall;
    ...
    function filterPathValue(path2) {
    ...
    function setupMSVCDevCmd2(arch, sdk, toolset, uwp, spectre, vsversion) {
      if (process2.platform != "win32") {
        core2.info("This is not a Windows virtual environment, bye!");
        return;
      }
      process2.env.PATH += path.delimiter + VSWHERE_PATH;
      let arch_aliases = {
        "win32": "x86",
        "win64": "x64",
        "x86_64": "x64",
      ...
      core2.debug(`vcvars command-line: ${vcvars}`);
      ...
      core2.startGroup("Environment variables");
      for (let string of new_environment) {
        ...
        if (new_value !== old_value) {
          core2.info(`Setting ${name}`);
          if (isPathVariable(name)) {
            new_value = filterPathValue(new_value);
          }
          core2.exportVariable(name, new_value);
        }
      }
      core2.endGroup();
      core2.info(`Configured Developer Command Prompt`);
    }
    exports2.setupMSVCDevCmd = setupMSVCDevCmd2;
  }
});

// index.js
var { setupMSVCDevCmd } = require_lib2();
